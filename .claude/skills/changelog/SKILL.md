---
name: changelog
description: Generate or update CHANGELOG.md from git history. Run before a release to document what changed.
argument-hint: "[version]"
---

Update `CHANGELOG.md` with a new entry based on git history since the last
documented release. Uses [Keep a Changelog](https://keepachangelog.com) format.

## Arguments

- `$ARGUMENTS` is an optional version string (e.g. `0.7.0`).
...
into these categories (omit empty categories):
