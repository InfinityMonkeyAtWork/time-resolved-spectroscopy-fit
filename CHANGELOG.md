...
- **The fit-archive query layer: handles, variants, diffs, labels, selection, pruning.** Every fit slot carries a stored 64-hex `handle` (`sha256(optimization_hash + file_name)`); any unambiguous prefix names one exact run, git-style. `FitResults.compare_models` gains a short `handle` column; new `FitResults.variants()` tabulates how the runs of one `(file, model, fit_type)` group differ in their *inputs* (constant columns suppressed; corrections shown as content digests, so value-only `dark` changes are visible); new `FitResults.diff(a, b)` reports pairwise input/output differences at the archive's equivalence tolerances — joint fits diff at the **bundle level** via the combined table (projection references escalate; whole-objective metric rows are gated by the joint comparability key). New `FitResults.set_label(ref, label)` sets the archive's one mutable display field post-hoc (projection references escalate to the joint record; labels persist through `save_fits` and work anywhere a handle prefix does). Every single-slot accessor and plot method (`get_parameters` / `get_correlations` / `get_confidence_intervals` / `get_mcmc` / `plot_fit` / `plot_mcmc` / `plot_sbs_slices` / `plot_param_evolution`, plus the `File.*` sugar) accepts `handle=` to pin an exact run, mutually exclusive with the `file`/`model`/`fit_type` filters; the `File.*` wrappers additionally verify the handle names one of *that file's* fits. `save_fits` / `export_fits` (and the `File.save_fit` / `File.export_fit` sugar) gain `select=` — `"all"`, `"latest"`, `"best"` + `by=`, or a handle prefix / label naming exact fits (selecting any slot of a joint bundle archives the whole bundle) — and new `Project.drop_fits(ref)` prunes a slot (all its re-runs) or a whole joint bundle from the in-session history; a lone projection refuses to drop. `select="best"` offers exactly the settled criteria — `aic` / `bic` / `chi2_red_raw` minimize, `chi2_red` ranks by |x − 1| (distance from the noise floor) — and refuses rankings that would silently pick a wrong winner: raw χ² and r² are not offered (more free parameters almost always win on them), groups spanning multiple fit views refuse to rank, and σ-scaled ranking over mixed `sigma_eff` raises. `compare_models` applies the same σ tiers (mixed-σ groups drop σ-scaled columns from the dynamic defaults; explicit requests raise naming both σ values) and drops default metric columns that are undefined on every matched row.
- **Optimizer seed**: `fit_wrapper` (and every fit method, via `**fit_wrapper_kwargs`) accepts `seed=` — a pure passthrough to the stage-1 optimizer for stochastic methods (`differential_evolution`, `basinhopping`, ...), recorded in `fit_settings` and keyed into fit identity only when supplied, so a seeded re-run dedups to one slot while seeded and unseeded runs stay distinct variants. No capability table: a seed on a method that cannot consume it surfaces as SciPy's own `TypeError`. Unrelated to SbS `seed_source`/`seed_values`, which choose initial parameter values.

- **Project-level joint fits are a first-class in-memory record** (decisions in [docs/design/archive/joint_fit_result.md](docs/design/archive/joint_fit_result.md)): every successful `Project.fit_2d` now captures one `JointFitResult` — the combined parameter table in optimizer order, one `JointFitProjection` per file carrying a total combined→local parameter map (data to look up, never `fileNN_` prefix parsing), joint `conf_ci` and `correl` (correlation over covariance: `stderr` in `params` recovers covariance), the joint MCMC posterior, and whole-objective metrics (`chi2_raw`/`chi2_red_raw`/`aic`/`bic` over the concatenated prediction, `chi2` as the sum of per-file σ-calibrated values, `r2` undefined) — published to a new append-only `Project._joint_fit_history` together with the per-file projection slots as one bundle (a capture failure publishes neither). `fit_2d` returns the record (ignoring it stays valid); `Project.results` snapshots both histories, and joint records get their own query surface — `FitResults.find_joint` / `get_joint` (with `files=` canonicalized and matched against the full participant set) and `plot_joint_mcmc`, which renders the joint chain through the same primitive as `plot_mcmc`. Iteration and `len()` on `FitResults` stay per-file-slot. A one-file project fit is still a joint result. The joint `.emcee()` chain — previously computed and thrown away — now survives and is also shown at fit time when `show_output >= 1`.
- `PlotConfig.to_json` / `PlotConfig.from_json`: canonical JSON (de)serialization for the schema-7 project payload — sorted keys for deterministic output, tuple-typed fields (`x_lim`/`y_lim`/`z_lim`/`panel_size`) validated as two-element numeric arrays and restored to tuples on read, unknown payload keys, non-standard JSON constants (`NaN`/`Infinity`), and non-JSON field values (arrays, callables, `NaN`/`inf`) all raise loudly naming the field instead of degrading, and missing keys keep their defaults so payloads written before a field existed still load.
- `MCMCResult` gains a `lnsigma` field — the final `__lnsigma` nuisance noise scale (`None` when the sampling was weighted) — built by the new single payload decoder `fit_io.mcmc_result_from_payload`, shared by `FitResults.get_mcmc` and `JointFitResult.mcmc`.
...
- `FitResults.get_parameters` / `get_correlations` / `get_confidence_intervals` / `get_mcmc`: the results accessors now live on `FitResults`, read the latest matching persisted slot (`file=` / `model=` / `fit_type=` filters), and therefore also work for SbS fits (slice-0 payloads) and on archives loaded with `FitResults.load`. The `File.get_*` methods remain as thin delegates with an added optional `model=` filter.
- **Explicit plotting API**: `FitResults.plot_fit` (observed/fit/residual for any fit type) and `FitResults.plot_param_evolution` (SbS per-parameter evolution, varied parameters by default), with `File.plot_fit` / `File.plot_param_evolution` sugar. `FitResults` now carries per-file axes providers (captured `SavedFile` records on both paths — see the captured-state entry under Changed), so plots — including the upgraded `plot_residuals` — use real energy/time axes on live sessions *and* loaded archives, falling back to array indices only when no provider matches. Styling resolves as explicit `config=` > the project-owned `plot_config` > defaults (styling is persisted per project since schema 7 — see Changed). The fit methods' inline display now routes through this same API, so the figure shown at fit time is exactly the figure the API reproduces later.
...
- `FitResults.plot_sbs_slices` (with `File.plot_sbs_slices` sugar): on-demand per-slice fit panels for the most recent Slice-by-Slice fit (slice data, per-slice seeded initial guess, final fit, component decomposition — more than the old auto-written per-slice PNGs showed), rendered from the persisted slot — works identically on live sessions and archives loaded via `FitResults.load`. `save_path=None` means display-only; pass a directory to write one PNG per slice, named `{slice_index:06d}.png`.
...
- **`FitResults.plot_fit` gains a `full_range` display mode** (`PlotConfig.full_range`, default `True`): shows the real full data/axis for the file, with fit/residual/components drawn only inside the fit window (`NaN` outside — never a fabricated value) and ROI boundary lines styled by `refline_color`/`refline_style`, matching `describe_model`'s visual language. All 4 fit methods' post-fit display now uses this mode by default.
...
- **A label now works with `handle=` too**: the single-fit accessors (`FitResults.get` / `get_parameters` / `plot_fit` / …, plus the `File.*` sugar) resolved `handle=` by prefix only, contradicting both the query layer's documented contract ("labels work anywhere a handle prefix does") and the resolver's own error message. `handle=` now accepts an exact label; `resolve_fit_reference` drops its unused `labels` switch. A joint bundle's label still doesn't resolve on slot accessors — bundle labels live on the joint record (use `get_joint`).
- **Correcting data after a fit no longer makes earlier results unsaveable**: `save_fits()`/`export_fits()` resolved each slot to its live file by requiring the content fingerprint to match, so `subtract_dark()`/`calibrate_data()` after a fit raised `"Slot for file ... has no matching Project.files entry"` and aborted the whole save. Slot→file lookup is now by the guarded `File.name`; the stored fingerprint remains as a version stamp recording what content the fit ran against. The archive stays internally consistent: the file payload's fingerprint is recomputed from the arrays actually written, and slots whose fit-time stamp no longer matches the file's current data are skipped with a warning naming each one — schema 6 stores one data payload per file, so a pre-correction fit cannot sit faithfully beside corrected data (restore the fit-time correction state or re-fit the current data to make them saveable; schema 7 — see Changed — retains per-fit correction state).
- **Appending to an archive can no longer file fits under another measurement's data**: `write_archive` matched file groups by name without checking content, so appending a same-name file with different data — a reused name, or a corrected file re-saved to the same path — silently stored the new slots under the old group's data, axes, and fingerprint. Same name + different content now raises `ValueError` in both `overwrite` modes before any mutation (`overwrite=` stays slot-scoped); a limitation schema 7 — see Changed — removes by storing immutable raw content with per-fit corrections.
- **Joint-fit projection slots no longer carry meaningless count-dependent metrics**: the per-file slots of a `Project.fit_2d` used to compute `chi2_red_raw`/`chi2_red`/`aic`/`bic` with the *joint* `nvarys`, which does not decompose by file. Those cells are now structurally `NaN` on projections (`compute_fit_metrics` accepts `n_free_pars=None`), while `chi2_raw`/`chi2`/`r2` stay valid; the joint record owns the whole-objective values. `compare_models` keeps the columns and shows `NaN` cells — independent-fit rows are unaffected — and its docs no longer claim `chi2_red_raw` is always populated.
- **`get_mcmc` dropped the persisted `lnsigma`** (lossy for every fit type): the slot payload stored it, but the returned `MCMCResult` had no field for it — see the `lnsigma` addition above.
- **Weighted MCMC no longer samples a meaningless `__lnsigma`**: `fit_wrapper` added the noise-scale nuisance parameter unconditionally, but lmfit only folds it into the log-probability for *unweighted* sampling — with `MC(is_weighted=True)` the chain carried a likelihood-free extra dimension whose reported "posterior" was noise. The nuisance is now added only when `is_weighted=False`; for weighted runs the chain and quantile table contain only model parameters and `MCMCResult.lnsigma` is `None`, matching its documented contract.
...
- **Breaking: fit-archive schema 7 — a fit is unique by its complete input, stored on disk** (`docs/design/fit_archive_principles.md`; wire format in `docs/design/fit_archive_schema.md`; conversion history in `docs/design/archive/fit_archive_schema_plan.md`). A clean break: the reader accepts only schema 7 — pre-7 archives are not readable (re-fit and re-save); the writer never appends across versions, and one archive holds one project. Every slot stores its identity — `handle`, `optimization_hash`, `input_files` (per-file version stamp + selection), `model_structure` (component composition and dynamics attachments incl. `frequency`), and the optimizer settings in force — so a changed bound, `vary` flag, fit window, seed, or data correction mints a **distinct variant that archives side by side** with the original, while byte-identical re-runs dedup to the latest per handle; the schema-6 `archive_slot_key`/`history_key`/`observed_sha256` trio is retired for the stored chain plus `fit_view_sha256` (comparability). The file group stores immutable `data_raw` once (single `file_content_hash`; gzip+shuffle compression on all array datasets) and each slot carries the `dark`/`calibration` state it was fit under — the schema-6 stale-slot skip warning is gone, corrected-data views are reconstructed on demand, and full-range plotting renders each slot against its own correction state. Joint fits persist as first-class `project/joint/` records (combined table, parameter maps, joint `conf_ci`/`correl`/MCMC, whole-objective metrics) cross-referenced by content hash (`joint_ref`), always saved as whole bundles, and rehydrated as the same `JointFitResult` type; joint projections omit the per-file count-dependent metrics (NaN on read). `PlotConfig` is persisted per project and loaded archives render with it; slots persist their model-YAML snippets verbatim (provenance, not rehydration) and an optional mutable `label`. Collision rules at the archive boundary and at in-session save collapse are one rule: divergent fitted values under one handle are a hard conflict (`overwrite=True` replaces; the message distinguishes a missing seed from a stochastic second stage), equivalent values merge attachments individually (absent→present enriches freely; stored never deletes; both-present replaces under `overwrite=True`), and refused appends leave the archive byte-untouched.
- **Completed-fit rendering reads captured state, never the live `File`.** `Project.results` now attaches each file's immutable captured `SavedFile` payload (with its history slots) as the axes/full-range provider — the same object type and data `FitResults.load` serves — so a completed fit renders identically before and after serialization, insulated from any later in-place mutation of the live file. Presentation stays the deliberate exception: the project-owned `PlotConfig` is passed live and resolved at render time. Live inspection/setup methods (`describe`, `describe_model`, `define_baseline`, `set_fit_limits`) keep reading live state — they are pre-fit tools.
- **Breaking (advanced API): all production rendering lives in `utils/plot.py`.** `fitlib.plt_fit_res_1d` / `plt_fit_res_2d` / `plt_fit_res_pars` are removed: the 2D data/fit/residual maps are `utils.plot.plot_fit_res_2d`, the SbS per-parameter series is `utils.plot.plot_par_series`, and `describe_model`'s single-panel overlay is `utils.plot.plot_fit_overlay_1d`, which takes plain curve arrays — model evaluation stays in `fitlib` as the new `eval_model_curves_1d`, so renderers never receive `File`/`Model`/lmfit objects. The MCMC walker/corner diagnostics and the side-by-side residual comparisons moved out of `FitResults` private methods into `utils.plot.plot_mcmc_diagnostics` / `plot_residual_panels_1d` / `plot_residual_maps_2d` (`fit_results` keeps slot selection, axes lookup, and titles), and SbS workers switch backends via `utils.plot.use_headless_backend`. No module under `src/trspecfit/` other than `utils/plot.py` imports matplotlib or corner — enforced by a source-boundary test. Public plotting APIs (`plot_fit`, `plot_mcmc`, `plot_residuals`, `describe_model`, …), returned-Figure contracts, and `show_plot`/`save_img` semantics are unchanged; under a **default** config the figures are too, except the MCMC diagnostics' dpi (hardcoded 75 → `dpi_plot`) and a now-labeled 2D colorbar. A **customized** `PlotConfig` reaches further than before — dpi, labels, limits, axis direction/scale, and the residual colormap now flow into every renderer that previously ignored them.
- **`File.load_model` overwrites a same-name model instead of raising.** The `ValueError` demanding a `delete_model` first predates the fit history — back then the live `Model` object was the only home of its results, so a silent replacement could destroy them. Completed fits are now captured into `Project._fit_history` at fit time, so re-loading a name just replaces the live object with a fresh YAML-seeded one (a `UserWarning` says so) — the natural idiom for what-if variants of one model. Un-fit state on the replaced object (parameter edits, attached dynamics never fit) is still discarded, hence the warning.
- **The comparison and persistence notebooks teach the query layer.** `10_model_comparison` gains §4: a pinned-`m` what-if run of one model walked through `variants()` (only the inputs that differ), `diff()`, `set_label`, and `drop_fits` (demonstrated on a junk run — the deliberate variant is kept). `11_save_load_export` picks that group up: `select=` (the `"all"` / `"latest"` defaults, `"best"` + `by=` ranking within a `(file, model, fit_type)` group, saving by the label set in notebook 10), and the archived variant group browsed from disk via `loaded.variants()` and a handle-pinned `get()`.
- **Breaking: result accessors renamed.** `get_fit_results` → `get_parameters` (it returns the fitted-parameter table; next to `FitResults.get()`, which returns the fit record, the old name read as the record accessor) and `get_conf_intervals` → `get_confidence_intervals`, on both `FitResults` and the `File.*` wrappers. The unreleased `FitResults.label(ref, label)` ships as `set_label(ref, label)` — the query layer's only mutating method should look like one. No deprecation shims: rename call sites when upgrading.
- **`save_fits` archives every variant by default; `export_fits` exports the latest.** The schema-6 collapse to latest-per-key is gone: distinct configurations no longer collide, so `save_fits` defaults to `select="all"` (the archive is the record) while `export_fits` defaults to `select="latest"` per `(file, model, fit_type)` group (the CSV/PNG tree is a curated presentation — pass `select="all"` for every variant, which then disambiguates directories with a handle suffix).
- **Breaking: presentation is project-owned — one `PlotConfig` per Project** (`docs/design/fit_archive_principles.md`, Principle 2). `Project.plot_config` is a real `PlotConfig` replacing the ~35 flat plot attributes (`project.e_label`, `project.z_colormap`, ... are gone — set fields on `project.plot_config` instead; `project.yaml` keys are unchanged, including the `e_label`/`t_label`/`dpi_plt` spellings and hyphenated forms, with tuple coercion for limit fields moved into config loading). Removed spellings fail loudly with migration pointers: writing a flat plot attribute (`project.full_range = ...`) or `file.plot_config` raises `AttributeError` naming the replacement, `Project.plot_config` rejects non-`PlotConfig` assignment (including a nested `plot_config:` mapping in `project.yaml`) and adopts valid assignments in place so held `FitResults` views keep resolving current styling, `PlotConfig.copy()` is now a deep copy, `File.plot_sbs_slices` gains the same per-call `config=` override as its siblings, and `write_csv_export` rejects the removed per-file config dict before touching the filesystem. `File.plot_config` and `PlotConfig.from_project` are deleted: models and files resolve the project config at render time, `FitResults` receives it at construction (`Project.results` passes the live one; since schema 7 persists styling, loaded archives render with the archived project config), and `export_fits` styles every file with the one project config.
- **Breaking: identity is a guarded name; content hashes are version stamps** (`docs/design/fit_archive_principles.md`, Principle 1). `Project.name`, `File.name`, and `Model.name` are immutable after construction (reassignment raises; a `name:` key in `project.yaml` is rejected). Content hashes stay as version stamps recording what data a fit ran against — a fit against corrected data is a distinct fit that survives snapshot collapse — while file *lookup* no longer uses them (the stored identity chain lives in the schema-7 entry above). `FitResults` resolves axes providers by parent association only (each `SavedFile` owns its slots, so one record's axes can never serve another record's slots); archive appends locate file groups by name instead of fingerprint.
...
- **Breaking: `Project.ext`, `Project.da_fmt`, and `Project.da_slices_fmt`.** All three were dead settings — defined and printed but never consumed by any export path. A `project.yaml` that still sets any of them fails loudly at load with migration guidance, matching `auto_export`/`path_results`.
