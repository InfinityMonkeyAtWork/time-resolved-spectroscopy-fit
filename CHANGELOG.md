...
- **Fit slots persist each file's full, uncropped axes** (archive schema 5): `SavedFile` gains `aux_axis` (the full energy or time array, whichever the fit didn't crop), enabling `FitResults.plot_fit`'s new `full_range` display mode below without re-deriving it from the fit window.
