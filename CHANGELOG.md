...
- **Explicit plotting API**: `FitResults.plot_fit` (observed/fit/residual for any fit type) and `FitResults.plot_param_evolution` (SbS per-parameter evolution, varied parameters by default), with `File.plot_fit` / `File.plot_param_evolution` sugar. `FitResults` now carries name-matched axes providers (live `File`s via `Project.results`, `SavedFile`s via `FitResults.load`), so plots — including the upgraded `plot_residuals` — use real energy/time axes on live sessions *and* loaded archives, falling back to array indices only when no provider matches. Styling resolves as explicit `config=` > the project-owned `plot_config` > defaults (styling is persisted per project since schema 7 — see Changed). The fit methods' inline display now routes through this same API, so the figure shown at fit time is exactly the figure the API reproduces later.
