...
- **Examples reorganized around user-workflow tracks.** Layout switches from a linear "walk forward" sequence to a flat directory under `examples/fitting_workflows/` with a numeric-block legend: **0x** = single-file fitting skills, **1x** = post-fit work, **2x** = multi-file workflows. Renames: `03_multi_cycle` → `03_multi_cycle_dynamics`, `04_par_profiles` → `04_parameter_profiles`, `05_project_level_fitting` → `21_project_level_shared_fit`. `examples/data_generation/` → `examples/synthetic_data/` (with `simulator` → `01_simulator`, `ml_training` → `02_ml_training_data`). Notebook `01_basic_fitting` extended with `get_fit_results` / `export_fit` / `save_fit` + the `auto_export` opt-out as the casual-user exit path. Notebook `10_model_comparison` trimmed to comparison-only; persistence and CSV/PNG export move to a new `11_save_load_export/` whose preamble re-runs notebook 10 in-kernel via `%%capture` + `%cd` + `%run example.ipynb` (single source of truth for the fitted state; notebook 10 keeps `show_output: 1` for direct interactive use). New `20_multi_file_independent_fit/` bridges single-file basics and the shared-parameter fit in `21_project_level_shared_fit`. See `examples/fitting_workflows/README.md` for the legend and `docs/design/examples_upgrade.md` for the design.
- Benchmark harness (`docs/ai/benchmark.md`, `.claude/skills/benchmark/benchmark_gir.py`) updated for the new example numbering: batch mode (`--example 0`) iterates examples 1–4; project-level fitting now lives at `21_project_level_shared_fit` and remains outside the harness.
