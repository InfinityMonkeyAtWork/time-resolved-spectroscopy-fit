...
- **Breaking: identity is a guarded name; content hashes are version stamps** (`docs/design/fit_archive_principles.md`, Principle 1). `Project.name`, `File.name`, and `Model.name` are immutable after construction (reassignment raises; a `name:` key in `project.yaml` is rejected). `SavedFitSlot.history_key` is composed from `(file_name, model_name, fit_type, selection_json)` — the fingerprint no longer participates, so refits after a data correction collapse onto the same key. `FitResults` matches axes providers and archive appends locate file groups by name instead of fingerprint.
