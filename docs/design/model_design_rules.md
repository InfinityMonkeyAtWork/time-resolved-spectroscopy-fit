...
- Energy model with a profile on a top-level parameter and dynamics on one of that profile's internal parameters.
...
- Expressions where a top-level energy expression parameter references a top-level profiled parameter whose internal profile parameters are themselves time-dependent, so the top-level expression inherits the resulting profile variation.
